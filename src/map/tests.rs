...
fn concurrent_insertion() {
    ...
    let map = std::sync::Arc::new(HashMap::with_capacity(MAX_INSERTED_VALUE as usize));
...
fn concurrent_overlapped_insertion() {
    ...
    let map = std::sync::Arc::new(HashMap::with_capacity(MAX_VALUE as usize));
    let barrier = std::sync::Arc::new(std::sync::Barrier::new(NUM_THREADS));
...
fn concurrent_overlapped_growth() {
    ...
    let map = std::sync::Arc::new(HashMap::with_capacity(1));
    let barrier = std::sync::Arc::new(std::sync::Barrier::new(NUM_THREADS));
...

    {
        ...
        assert_eq!(
            map.insert_and(
                ...
                crate::test_util::NoisyDropper::new(std::sync::Arc::clone(&value_parent), 0),
                |_| ()
            ),
            None
        );
...
fn drop_many_values() {
    ...
    let key_parents: Vec<_> =
        std::iter::repeat_with(|| std::sync::Arc::new(crate::test_util::DropNotifier::new()))
            .take(NUM_VALUES)
            .collect();
...

    {
            ...
            key_parents.iter().zip(value_parents.iter()).enumerate()
        {
            assert_eq!(
                map.insert_and(
                    crate::test_util::NoisyDropper::new(std::sync::Arc::clone(&this_key_parent), i),
...
#[test]
fn default() {
    let map = HashMap::<_, _, crate::map::DefaultHashBuilder>::default();
